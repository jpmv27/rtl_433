...
static int ecowitt_decode(r_device *decoder, bitbuffer_t *bitbuffer)
{
    ...
    // All Ecowitt packets should have 55 bits. For some reason, sometimes
    // packets only have 54 bits, with a header that is 6 bits instead of 7.
    ...
    else if (bitbuffer->bits_per_row[0] == 54) {
        // Short packet case
        header_length = 6;
    }
    else {
    ...
    // All Ecowitt packets should have all header bits set. But some packets
    // have the first bit clear and the other bits set. Only check that the
    // bottom five bits are set, in case header is only 6 bits long.

    uint8_t header_bits = row[0] >> (8 - header_length);
    if ((header_bits & 0x1F) != 0x1F) {
        return DECODE_ABORT_EARLY;
    }

    // Byte-align the rest of the message by skipping the header.
    uint8_t b[6];
    bitbuffer_extract_bytes(bitbuffer, /* row= */ 0, /* pos= */ header_length, b, sizeof(b) * 8); // Skip header

    // All Ecowitt packets continue with a fixed model code
    if (b[0] != 0x53) {
        return DECODE_ABORT_EARLY;
    }
    ...
    // Compute checksum skipping the header bits
